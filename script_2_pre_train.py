...
from code.MethodGraphBertJointPreTrain import MethodGraphBertJointPreTrain
...
#---- 两个预训练task联合预训练Pre-Training Task #0: Graph Bert Node Attribute Reconstruction And  Network Structure Recovery(Cora, Citeseer, and Pubmed) ----
if 1:
    #---- hyper-parameters ----
    ...
    method_obj = MethodGraphBertJointPreTrain(bert_config)
    method_obj.max_epoch = max_epoch
    method_obj.lr = lr

    result_obj = ResultSaving()
    result_obj.result_destination_folder_path = './result/GraphBert/'
    result_obj.result_destination_file_name = dataset_name + '_' + str(k) + '_joint_pretrain'
    ...
    # ------------------------------------------------------
    print("保存联合预训练的预训练完整模型")
    # method_obj.save_pretrained('./result/PreTrained_GraphBert/' + dataset_name + '/Joint_PreTrain_complete_model/')
    print('************ Finish ************')
#------------------------------------
